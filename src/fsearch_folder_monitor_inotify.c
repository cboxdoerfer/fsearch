...
static void
unwatch_folder_cb(gpointer key, gpointer value, gpointer user_data);

static void
unwatch_all(FsearchFolderMonitorInotify *self) {
    g_autoptr(GMutexLocker) locker = g_mutex_locker_new(&self->mutex);
    g_assert_nonnull(locker);

    g_hash_table_foreach(self->watch_descriptors_to_folders, (GHFunc)unwatch_folder_cb, self);
    g_hash_table_remove_all(self->watch_descriptors_to_folders);
    g_hash_table_remove_all(self->watched_folders_to_descriptors);
}
...
static void
unwatch_folder(FsearchDatabaseEntry *folder, int fd, const int32_t wd) {
