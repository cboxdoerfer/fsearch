...
test_parse_time_range(void) {
    ...
    } FsearchTestTimeRangeParseContext;

    FsearchTestTimeRangeParseContext strings[] = {
    ...
    for (gint i = 0; i < G_N_ELEMENTS(strings); ++i) {
        FsearchTestTimeRangeParseContext *ctx = &strings[i];
        time_t time_start = 0;
        time_t time_end = 0;
        char *end_ptr = NULL;
        gboolean res = fsearch_time_parse_range(ctx->string, &time_start, &time_end, &end_ptr);
...
main(int argc, char *argv[]) {
    ...
    g_test_add_func("/FSearch/time_utils/parse_time_range", test_parse_time_range);
    return g_test_run();
}
