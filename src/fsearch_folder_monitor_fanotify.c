...
static void
unwatch_all(FsearchFolderMonitorFanotify *self) {
    g_autoptr(GMutexLocker) locker = g_mutex_locker_new(&self->mutex);
    g_assert_nonnull(locker);

    g_hash_table_foreach(self->handles_to_folders, (GHFunc)unwatch_folder_cb, self);
    g_hash_table_remove_all(self->folders_to_handles);
    g_hash_table_remove_all(self->handles_to_folders);
}
