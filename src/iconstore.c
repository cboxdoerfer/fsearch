...
get_themed_icon_pixbuf(GThemedIcon *icon, int size, GtkIconTheme *icon_theme) {
    ...
    GtkIconInfo *icon_info = gtk_icon_theme_choose_icon(
        icon_theme, (const char **)icon_names, size, GTK_ICON_LOOKUP_FORCE_SIZE);
    if (icon_info == NULL) {
        icon_info = gtk_icon_theme_lookup_icon(
            icon_theme, "text-x-generic", size, GTK_ICON_LOOKUP_FORCE_SIZE);
    }
...
iconstore_get_pixbuf(GFileInfo *file_info) {
    ...
    GdkPixbuf *pixbuf = (GdkPixbuf *)g_hash_table_lookup(pixbuf_hash_table, icon_string);

    if (pixbuf == NULL) {
        pixbuf =
            get_themed_icon_pixbuf(G_THEMED_ICON(icon), ICON_SIZE, gtk_icon_theme_get_default());
