...
static void
on_exclude_model_modified (GtkTreeModel *model,
                           GtkTreePath  *path,
                           gpointer      user_data)
{
...
static void
on_include_model_modified (GtkTreeModel *model,
                           GtkTreePath  *path,
                           gpointer      user_data)
{
    FsearchPreferences *pref = user_data;
...
static void
column_text_append (GtkTreeView *view, const char *name, gboolean expand, int pos)
{
...
static void
on_column_exclude_toggled (GtkCellRendererToggle *cell,
                           gchar                 *path_str,
                           gpointer               data)
{
...
static void
on_column_include_enable_toggled (GtkCellRendererToggle *cell,
                                  gchar                 *path_str,
                                  gpointer               data)
{
    FsearchPreferences *pref = data;
...
static void
on_column_include_toggled (GtkCellRendererToggle *cell,
                           gchar                 *path_str,
                           gpointer               data)
{
    FsearchPreferences *pref = data;
...
static void
column_toggle_append (GtkTreeView *view, GtkTreeModel *model, const char *name, int pos, GCallback cb, gpointer user_data)
{
...
pref_include_treeview_data_get (GtkTreeView *view)
{
    ...
    while (valid) {
        ...
        gtk_tree_model_get (model, &iter, COL_INCLUDE_ENABLE, &enable, COL_INCLUDE_PATH, &path, COL_INCLUDE_UPDATE, &update, -1);
...
pref_include_treeview_row_add (FsearchPreferences *pref, const char *path)
{
    ...
    gtk_list_store_set (GTK_LIST_STORE (pref->include_model), &iter,
                        COL_INCLUDE_ENABLE, fs_path->enabled,
                        COL_INCLUDE_PATH, fs_path->path,
                        COL_INCLUDE_UPDATE, fs_path->update,
                        -1);
    pref->update_db = true;
}
...
pref_exclude_treeview_row_add (FsearchPreferences *pref, const char *path)
{
    ...
    gtk_list_store_set (GTK_LIST_STORE (pref->exclude_model), &iter,
                        COL_EXCLUDE_ENABLE, fs_path->enabled,
                        COL_EXCLUDE_PATH, fs_path->path,
                        -1);
    pref->update_db = true;
}
...
{
    GtkListStore *store = gtk_list_store_new (NUM_INCLUDE_COLUMNS,
                                              G_TYPE_BOOLEAN,
                                              G_TYPE_STRING,
                                              G_TYPE_BOOLEAN);
    gtk_tree_view_set_model (view, GTK_TREE_MODEL (store));

    column_toggle_append (view, GTK_TREE_MODEL (store), "Enable", COL_INCLUDE_ENABLE, G_CALLBACK (on_column_include_enable_toggled), pref);
    column_text_append (view, "Path", TRUE, COL_INCLUDE_PATH);
    column_toggle_append (view, GTK_TREE_MODEL (store), "Update", COL_INCLUDE_UPDATE, G_CALLBACK (on_column_include_toggled), pref);
    ...
    g_signal_connect (pref->include_model,
                      "row-changed",
                      G_CALLBACK (on_include_model_modified),
                      (gpointer)pref);
    g_signal_connect (pref->include_model,
                      "row-deleted",
                      G_CALLBACK (on_include_model_modified),
...
{
    GtkListStore *store = gtk_list_store_new (NUM_EXCLUDE_COLUMNS,
                                              G_TYPE_BOOLEAN,
                                              G_TYPE_STRING);
    gtk_tree_view_set_model (view, GTK_TREE_MODEL (store));

    column_toggle_append (view, GTK_TREE_MODEL (store), "Enable", COL_EXCLUDE_ENABLE, G_CALLBACK (on_column_exclude_toggled), pref);
    column_text_append (view, "Path", TRUE, COL_EXCLUDE_PATH);
    ...
    g_signal_connect (pref->exclude_model,
                      "row-changed",
                      G_CALLBACK (on_exclude_model_modified),
                      pref);
    g_signal_connect (pref->exclude_model,
                      "row-deleted",
                      G_CALLBACK (on_exclude_model_modified),
